...
class _MatchListPageState extends State<MatchListPage> {
  ...
  Future<void> _deleteMatch(CookieRequest request, int matchId) async {
    final url = '$baseUrl/tournament/api/${widget.tournament.id}/matches/$matchId/delete/';
      ...
      if (response['status'] == 'success') {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text("Pertandingan berhasil dihapus"), backgroundColor: Colors.red),
          );
          setState(() {}); 
        }
      } else {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text("Gagal: ${response['message']}"), backgroundColor: Colors.grey),
          );
        }
      }
    } 
  ...
  // EDIT SCORE
  Future<void> _editMatchScore(CookieRequest request, int matchId, int score1, int score2) async {
    final url = '$baseUrl/tournament/api/${widget.tournament.id}/matches/$matchId/edit/';
    ...
    try {
      ...
      if (response['status'] == 'success') {
        if (mounted) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text("Skor berhasil diupdate"), backgroundColor: Colors.green),
          );
          // REFRESH
          setState(() {}); 
        }
  ...
  void _showEditDialog(CookieRequest request, TournamentMatch match) {
    final TextEditingController score1Controller = TextEditingController(text: match.score1.toString());
    final TextEditingController score2Controller = TextEditingController(text: match.score2.toString());
      ...
      builder: (context) {
        return AlertDialog(
          ...
          content: Column(
            ...
            children: [
              Text("${match.team1} vs ${match.team2}", style: const TextStyle(color: Colors.grey)),
    ...
    showDialog(
      ...
      builder: (context) {
        return AlertDialog(
          ...
          content: const Text("Apakah Anda yakin ingin menghapus match ini? Data tidak bisa dikembalikan."),
          actions: [
            TextButton(
              onPressed: () => Navigator.pop(context),
              child: const Text("Batal", style: TextStyle(color: Colors.grey)),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text("Match List", style: TextStyle(color: Colors.black, fontWeight: FontWeight.bold)),
        backgroundColor: Colors.white,
        elevation: 0,
        leading: IconButton(
        ...
        children: [
          ...
          Row(
            children: [
              ...
              Expanded(child: Text(match.team1, style: TextStyle(color: Colors.grey[600], fontSize: 14))),
              Text("${match.score1}", style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 18)),
            ],
          ),
          // VS
          const Padding(padding: EdgeInsets.symmetric(vertical: 8.0), child: Center(child: Text("vs", style: TextStyle(color: Colors.grey, fontSize: 12)))),
          // TEAM 2
          Row(
            children: [
              const CircleAvatar(radius: 16, backgroundColor: Colors.grey, child: Icon(Icons.person, size: 20, color: Colors.white)),
              const SizedBox(width: 12),
              Expanded(child: Text(match.team2, style: TextStyle(color: Colors.grey[600], fontSize: 14))),
              Text("${match.score2}", style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 18)),
            ],
          ),
          ...
          if (widget.tournament.isCreator)
              ...
              children: [
                InkWell(
                  ...
                  child: const Padding(
                    ...
                    child: Text("Edit Score", style: TextStyle(color: Color(0xFFFFD600), fontWeight: FontWeight.bold)),
                  ),
                ),
                InkWell(
                  ...
                  child: const Padding(
                    padding: EdgeInsets.all(8.0),
                    child: Text("Delete Match", style: TextStyle(color: Colors.red, fontWeight: FontWeight.bold)),
                  ),
                ),
              ],
            )
            ...
            const Center(
              child: Text("Read Only", style: TextStyle(color: Colors.grey, fontSize: 10, fontStyle: FontStyle.italic)),
            ),
        ],
      ),
    );
